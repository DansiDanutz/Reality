...
CREATE UNIQUE INDEX IF NOT EXISTS ledger_citizen_idempotency_key
  ON ledger (citizen_id, idempotency_key) WHERE idempotency_key IS NOT NULL;
...
-- Final rate gate inside the same citizen advisory lock. The handler keeps
-- its early check for cheap rejection, but this gate closes the concurrent
-- check-then-append race and records a refusal for audit/rate accounting.
CREATE OR REPLACE FUNCTION reality_append_intent(p_citizen uuid, p_intent text, p_amount numeric, p_seed numeric, p_meta jsonb, p_cooldown interval) RETURNS TABLE (new_balance numeric, refusal text) LANGUAGE plpgsql AS $reality$ DECLARE v_base numeric; BEGIN PERFORM pg_advisory_xact_lock(hashtext(p_citizen::text)); PERFORM reality_ensure_genesis(p_citizen); SELECT l.balance_after INTO v_base FROM ledger l WHERE l.citizen_id = p_citizen AND l.balance_after IS NOT NULL ORDER BY l.entry_id DESC LIMIT 1; IF (SELECT count(*) FROM ledger l WHERE l.citizen_id = p_citizen AND l.intent LIKE 'intent.%' AND l.created_at > now() - interval '1 hour') >= 120 THEN INSERT INTO ledger (citizen_id, intent, amount, balance_after, meta) VALUES (p_citizen, p_intent, 0, NULL, p_meta || jsonb_build_object('refusal', 'rate_limited')); RETURN QUERY SELECT v_base, 'rate_limited'::text; RETURN; END IF; IF p_cooldown IS NOT NULL AND EXISTS (SELECT 1 FROM ledger l WHERE l.citizen_id = p_citizen AND l.intent = p_intent AND l.balance_after IS NOT NULL AND l.created_at > now() - p_cooldown) THEN INSERT INTO ledger (citizen_id, intent, amount, balance_after, meta) VALUES (p_citizen, p_intent, 0, NULL, p_meta || jsonb_build_object('refusal', 'cooldown')); RETURN QUERY SELECT NULL::numeric, 'cooldown'::text; RETURN; END IF; IF v_base + p_amount < 0 THEN INSERT INTO ledger (citizen_id, intent, amount, balance_after, meta) VALUES (p_citizen, p_intent, 0, NULL, p_meta || jsonb_build_object('refusal', 'insufficient_funds')); RETURN QUERY SELECT v_base, 'insufficient_funds'::text; RETURN; END IF; INSERT INTO ledger (citizen_id, intent, amount, balance_after, meta) VALUES (p_citizen, p_intent, p_amount, v_base + p_amount, p_meta); RETURN QUERY SELECT v_base + p_amount, NULL::text; END $reality$;
...
-- Cross-instance Overpass admission. Keys are SHA-256 IP prefixes, never raw
-- addresses. One advisory lock makes the rolling-window counters atomic.
CREATE TABLE IF NOT EXISTS overpass_rate_limits (scope text NOT NULL, scope_key text NOT NULL, window_started_at timestamptz NOT NULL, requests integer NOT NULL DEFAULT 0, PRIMARY KEY (scope, scope_key));
CREATE OR REPLACE FUNCTION reality_claim_overpass_slot(p_ip_hash text, p_now timestamptz, p_ip_limit integer, p_global_limit integer) RETURNS text LANGUAGE plpgsql AS $reality$ DECLARE v_ip integer; v_global integer; v_window timestamptz := p_now - interval '60 seconds'; BEGIN PERFORM pg_advisory_xact_lock(hashtext('overpass:' || date_trunc('minute', p_now)::text)); EXECUTE 'DELETE ' || 'FROM overpass_rate_limits WHERE window_started_at < $1' USING v_window; SELECT requests INTO v_ip FROM overpass_rate_limits WHERE scope = 'ip' AND scope_key = p_ip_hash; IF COALESCE(v_ip, 0) >= p_ip_limit THEN RETURN 'ip'; END IF; SELECT requests INTO v_global FROM overpass_rate_limits WHERE scope = 'global' AND scope_key = 'all'; IF COALESCE(v_global, 0) >= p_global_limit THEN RETURN 'global'; END IF; INSERT INTO overpass_rate_limits (scope, scope_key, window_started_at, requests) VALUES ('ip', p_ip_hash, p_now, 1) ON CONFLICT (scope, scope_key) DO UPDATE SET window_started_at = CASE WHEN overpass_rate_limits.window_started_at < v_window THEN EXCLUDED.window_started_at ELSE overpass_rate_limits.window_started_at END, requests = CASE WHEN overpass_rate_limits.window_started_at < v_window THEN 1 ELSE overpass_rate_limits.requests + 1 END; INSERT INTO overpass_rate_limits (scope, scope_key, window_started_at, requests) VALUES ('global', 'all', p_now, 1) ON CONFLICT (scope, scope_key) DO UPDATE SET window_started_at = CASE WHEN overpass_rate_limits.window_started_at < v_window THEN EXCLUDED.window_started_at ELSE overpass_rate_limits.window_started_at END, requests = CASE WHEN overpass_rate_limits.window_started_at < v_window THEN 1 ELSE overpass_rate_limits.requests + 1 END; RETURN NULL; END $reality$;
...
-- The non-zero revision arm updates only an existing row; this prevents a
-- delete/recreate race from resurrecting stale state. Keep the function body
-- on one line because the migration runner splits statements on ";\\n".
CREATE OR REPLACE FUNCTION reality_save_founder_area(p_citizen uuid, p_area_id text, p_expected_revision bigint, p_state jsonb, p_simulation_at timestamptz, p_updated_at timestamptz) RETURNS bigint LANGUAGE plpgsql AS $reality$ DECLARE v_revision bigint; BEGIN IF p_expected_revision = 0 THEN INSERT INTO founder_area_snapshots (citizen_id, area_id, revision, state, simulation_at, updated_at) VALUES (p_citizen, p_area_id, 1, p_state, p_simulation_at, p_updated_at) ON CONFLICT (citizen_id) DO UPDATE SET area_id = EXCLUDED.area_id, revision = founder_area_snapshots.revision + 1, state = EXCLUDED.state, simulation_at = EXCLUDED.simulation_at, updated_at = EXCLUDED.updated_at WHERE founder_area_snapshots.revision = 0 RETURNING revision INTO v_revision; ELSE UPDATE founder_area_snapshots SET area_id = p_area_id, revision = founder_area_snapshots.revision + 1, state = p_state, simulation_at = p_simulation_at, updated_at = p_updated_at WHERE citizen_id = p_citizen AND revision = p_expected_revision RETURNING revision INTO v_revision; END IF; IF NOT FOUND THEN RAISE EXCEPTION 'founder_area_revision_conflict'; END IF; RETURN v_revision; END $reality$;

-- Cloud saves are ordered in Postgres before the Blob compatibility mirror is
-- written. Restore reads this row first, so a delayed stale Blob write cannot
-- clobber a newer save on another device.
...
CREATE OR REPLACE FUNCTION reality_save_cloud_snapshot(p_citizen uuid, p_saved_at bigint, p_state jsonb) RETURNS boolean LANGUAGE plpgsql AS $reality$ DECLARE v_accepted boolean; BEGIN INSERT INTO cloud_saves (citizen_id, saved_at, state) VALUES (p_citizen, p_saved_at, p_state) ON CONFLICT (citizen_id) DO UPDATE SET saved_at = EXCLUDED.saved_at, state = EXCLUDED.state, updated_at = now() WHERE cloud_saves.saved_at < EXCLUDED.saved_at RETURNING true INTO v_accepted; RETURN COALESCE(v_accepted, false); END $reality$;
...
-- Idempotent intent append overload. The legacy six-argument function remains
-- for old migrations; new callers supply a key so retries replay the exact
-- prior result instead of charging or paying twice.
CREATE OR REPLACE FUNCTION reality_append_intent(p_citizen uuid, p_intent text, p_amount numeric, p_seed numeric, p_meta jsonb, p_cooldown interval, p_idempotency_key text) RETURNS TABLE (new_balance numeric, refusal text) LANGUAGE plpgsql AS $reality$ DECLARE v_base numeric; v_existing_balance numeric; v_existing_refusal text; v_existing boolean; BEGIN PERFORM pg_advisory_xact_lock(hashtext(p_citizen::text)); PERFORM reality_ensure_genesis(p_citizen); SELECT l.balance_after, l.meta->>'refusal' INTO v_existing_balance, v_existing_refusal FROM ledger l WHERE l.citizen_id = p_citizen AND l.idempotency_key = p_idempotency_key LIMIT 1; v_existing := FOUND; IF v_existing THEN RETURN QUERY SELECT v_existing_balance, v_existing_refusal; RETURN; END IF; SELECT l.balance_after INTO v_base FROM ledger l WHERE l.citizen_id = p_citizen AND l.balance_after IS NOT NULL ORDER BY l.entry_id DESC LIMIT 1; IF (SELECT count(*) FROM ledger l WHERE l.citizen_id = p_citizen AND l.intent LIKE 'intent.%' AND l.created_at > now() - interval '1 hour') >= 120 THEN INSERT INTO ledger (citizen_id, intent, amount, balance_after, meta, idempotency_key) VALUES (p_citizen, p_intent, 0, NULL, p_meta || jsonb_build_object('refusal', 'rate_limited'), p_idempotency_key); RETURN QUERY SELECT v_base, 'rate_limited'::text; RETURN; END IF; IF p_cooldown IS NOT NULL AND EXISTS (SELECT 1 FROM ledger l WHERE l.citizen_id = p_citizen AND l.intent = p_intent AND l.balance_after IS NOT NULL AND l.created_at > now() - p_cooldown) THEN INSERT INTO ledger (citizen_id, intent, amount, balance_after, meta, idempotency_key) VALUES (p_citizen, p_intent, 0, NULL, p_meta || jsonb_build_object('refusal', 'cooldown'), p_idempotency_key); RETURN QUERY SELECT NULL::numeric, 'cooldown'::text; RETURN; END IF; IF v_base + p_amount < 0 THEN INSERT INTO ledger (citizen_id, intent, amount, balance_after, meta, idempotency_key) VALUES (p_citizen, p_intent, 0, NULL, p_meta || jsonb_build_object('refusal', 'insufficient_funds'), p_idempotency_key); RETURN QUERY SELECT v_base, 'insufficient_funds'::text; RETURN; END IF; INSERT INTO ledger (citizen_id, intent, amount, balance_after, meta, idempotency_key) VALUES (p_citizen, p_intent, p_amount, v_base + p_amount, p_meta, p_idempotency_key); RETURN QUERY SELECT v_base + p_amount, NULL::text; END $reality$;
