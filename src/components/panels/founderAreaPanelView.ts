...
export function founderCovenantOperatorQueueReviewRows(
  ...
  return queue.items
    .map((item) => ({
      ...
      reviewQueueBlockerText: founderCovenantOperatorQueueReviewQueueBlockerText(item),
...
export function founderCovenantOperatorQueueNextActionText(
  item: Pick<RealityFounderCovenantReviewQueueItem, 'nextAction'>,
): string {
  return founderCovenantNextActionLabel(item.nextAction)
}

export function founderCovenantOperatorQueueReviewQueueBlockerText(
  item: Pick<RealityFounderCovenantReviewQueueItem, 'reviewQueue'>,
): string {
  if (item.reviewQueue.blockers.length === 0) return 'none'
  return item.reviewQueue.blockers.map(founderCovenantApprovalBlockerLabel).join(', ')
}
...
export function founderCovenantOperatorQueueStageReasonText(
  item: Pick<RealityFounderCovenantReviewQueueItem, 'stages'>,
): string {
  if (item.stages.length === 0) return 'none'
  return item.stages.map((stage) => `${stage.label}: ${stage.reason}`).join(' · ')
}
...
export function founderCovenantOperatorQueueManualActionReasonText(
  item: Pick<RealityFounderCovenantReviewQueueItem, 'manualActions'>,
): string {
  const suggested = item.manualActions.filter((action) => action.recommended)
  if (suggested.length === 0) return 'none'
  return suggested.map((action) => `${founderCovenantManualActionKindLabel(action.kind)}: ${action.reason}`).join(' · ')
}
...
export function founderCovenantOperatorQueueNotificationDraftBodyText(
  item: Pick<RealityFounderCovenantReviewQueueItem, 'pendingNotificationDrafts'>,
): string {
  if (item.pendingNotificationDrafts.length === 0) return 'none'
...
export function founderCovenantOperatorQueueNotificationDraftGateText(
  ...
  if (item.pendingNotificationDrafts.length === 0) return 'none'
  return item.pendingNotificationDrafts
    .map((draft) => founderCovenantNotificationDraftGateText(draft))
    .join(' · ')
}

export function founderCovenantOperatorQueueNotificationDraftStatusText(
  item: Pick<RealityFounderCovenantReviewQueueItem, 'pendingNotificationDrafts'>,
): string {
  if (item.pendingNotificationDrafts.length === 0) return 'none'
  return item.pendingNotificationDrafts
