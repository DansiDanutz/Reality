  ...
  return (
    <section className="founder-section founder-covenant-operator-queue" aria-label="Founder covenant operator queue">
      ...
      <div className="founder-ledger-summary" aria-label="Founder operator queue totals">
        ...
        <QueueTotalChip
          label="Fresh"
          tone={sliceTotals.freshReviewed === 0 ? 'warning' : 'stable'}
          value={sliceTotals.freshReviewed}
        />
        <QueueTotalChip
          label="Stale"
          tone={sliceTotals.staleReviewed > 0 ? 'warning' : 'stable'}
          value={sliceTotals.staleReviewed}
        />
        <QueueTotalChip
          label="Stale week"
          tone={sliceTotals.staleWeeklyDue > 0 ? 'warning' : 'stable'}
          value={sliceTotals.staleWeeklyDue}
        />
        <QueueTotalChip
          label="Stale month"
          tone={sliceTotals.staleMonthlyDue > 0 ? 'critical' : 'stable'}
          value={sliceTotals.staleMonthlyDue}
        />
        <QueueTotalChip
          label="Weekly"
          ...
          {reviewRows.map((row) => (
            <li className={`item founder-covenant-checklist-item ${row.statusClass}`} key={row.key}>
              <div className="item-info">
                ...
                <span className="item-desc">Recommended next: {row.recommendedNextText}</span>
