...
describe('worldSim snapshot codec', () => {
  ...
  test('rejects invalid citizen state and business shape', () => {
    ...
    const hiddenDebt = area()
    delete hiddenDebt.citizens[0].debts
    expect(decodeWorldAreaSnapshot(JSON.stringify({ version: WORLD_AREA_SNAPSHOT_VERSION, area: hiddenDebt }))).toEqual({ ok: false, error: 'invalid_area' })
