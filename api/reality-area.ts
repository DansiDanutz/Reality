...
interface FounderCovenantOperatorReviewReceipt {
  recordedAt: string
  manualEvidenceKinds: readonly FounderAreaCovenantManualEvidenceKind[]
...
async function handleFounderCovenantOperatorReview(
  ...
  res.status(200).json({
    ...areaPayload(state),
    reviewReceipt: founderCovenantOperatorReviewReceipt(state, result.entry, result.evidenceKinds),
...
function applyRecordCovenantReviewIntent(
  ...
  entry: FounderAreaCovenantReviewHistoryItem
  evidenceKinds: FounderAreaCovenantManualEvidenceKind[]
} | { ok: false; error: ApplyRecordCovenantReviewError } {
...
function founderCovenantOperatorReviewReceipt(
): FounderCovenantOperatorReviewReceipt {
  const decision = entry.decision ?? {
    status: state.founderCovenant.status,
    nextAction: state.founderCovenant.nextAction,
    manualReviewRequired: state.founderCovenant.manualReviewRequired,
    id: entry.id,
    reviewerId: entry.reviewerId,
    recordedAt: entry.at,
    manualEvidenceKinds: [...evidenceKinds],
    manualReviewRequired: decision.manualReviewRequired,
    covenantStatus: decision.status,
    nextAction: decision.nextAction,
    pendingApprovalCount: entry.reviewQueue.pendingApprovalCount,
    pendingNotificationCount: entry.reviewQueue.pendingNotificationCount,
    blockerCount: entry.reviewQueue.blockerCount,
    blockers: [...entry.reviewQueue.blockers],
