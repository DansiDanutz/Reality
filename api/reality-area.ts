...
function founderCovenantReviewQueueTotals(
  ...
  return {
    ...
    warningDrafts: items.reduce((total, item) =>
      total + item.pendingNotificationDrafts.filter((draft) => draft.kind === 'founder_warning').length, 0),
...
function isFounderAreaState(value: unknown, citizenId: string): value is FounderAreaState {
  return isRecord(value) &&
    ...
    value.claim.founderCitizenId === citizenId &&
    value.claim.founderNumber === value.founderNumber &&
    typeof value.claim.label === 'string' &&
