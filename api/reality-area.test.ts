...
describe('reality area authority API', () => {
  ...
  test('tickAreas rejects persisted area states without citizens arrays', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-07-06T08:00:00.000Z'))
    const missingCitizens = { ...existingState(), citizens: undefined }
    vi.mocked(list)
      .mockResolvedValueOnce(blobList([areaStatePath(CITIZEN_ID)], 'blob://missing-citizens-clock-area'))
    vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify(missingCitizens), { status: 200 })))
    const res = responseRecorder()
