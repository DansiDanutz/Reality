  ...
  test('hireWorker staffs a server-owned business without letting the client set wages', async () => {
    ...
    expect((accepted.body as { dashboard: DashboardWithBuildGuidance }).dashboard).toMatchObject({
      ...
      existingBusinesses: [{
        ...
        status: 'stable',
        alerts: [],
  test('hireWorker returns a structured storage failure when accepted staffing cannot persist', async () => {
    ...
    expect(body.state.businesses[0].staffCitizenIds).toEqual([])
