...
describe('reality area authority API', () => {
  ...
  test('buyInsurance returns a structured storage failure when accepted policy cannot persist', async () => {
    const existing = withBusiness({ ...existingState(), updatedAt: '2026-07-06T08:00:00.000Z' }, {
    expect(body.state.businesses[0]).toMatchObject({ id: 'insurance-1', cash: 5 })
