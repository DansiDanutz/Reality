...
describe('reality area authority API', () => {
  ...
  test('tickAreas rejects persisted area states without updatedAt timestamps', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-07-06T08:00:00.000Z'))
    const missingUpdatedAt = { ...existingState(), updatedAt: undefined }
    vi.mocked(list)
      .mockResolvedValueOnce(blobList([areaStatePath(CITIZEN_ID)], 'blob://missing-updated-at-clock-area'))
    vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify(missingUpdatedAt), { status: 200 })))
    const res = responseRecorder()
