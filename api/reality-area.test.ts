...
describe('reality area authority API', () => {
  ...
  test('repayDebt returns a structured storage failure when accepted repayment cannot persist', async () => {
    expect(founder?.debts).toMatchObject([{ id: 'founder-medical-1', amount: 300, creditorId: 'clinic-1' }])
    expect(body.state.businesses[0]).toMatchObject({ id: 'clinic-1', cash: 10 })
