...
describe('reality area authority API', () => {
  ...
  test('recordCovenantReview appends manual evidence without touching money or ledger transactions', async () => {
    ...
    expect(body.state.founderCovenant.notificationDrafts).toEqual([{
      ...
      sendEnabled: false,
      authorityGate: mainFounderApprovalGate(),
  test('recordCovenantReview returns a structured storage failure when accepted evidence cannot persist', async () => {
    expect(body.state.founderReviewHistory).toBeUndefined()
    expect(body.state.founderCovenant.reviewHistory).toEqual([])
    expect(body.state.founderCovenant.latestReview).toBeNull()
